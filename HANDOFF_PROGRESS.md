...
- [x] **attr → 타일 인덱스** 확정: `material×16 + (variant & 0x0F)` (5맵 99.9%+)
- [x] **지형 타일 합성 렌더** + 플레이 1:1 타일 블릿
- [x] 기체 **char1–3.spr** 인게임 120프레임 yaw 연결
...
| 순위 | 작업 | 비고 |
|------|------|------|
| 1 | **SPR 원본 DD 팔레트 확정** | TIL DAC 공유는 적용됨 · 스크린샷 클러스터/런타임 셋 교차검증 |
| 2 | **무기·샷 SPR 인게임** | wp*/\*SHT · 장착 포인트 |
| 3 | **GM 사운드폰트 BGM** | Tone 신스 대신 FluidR3 등 (지금 tactics*.mid 재생 가능) |
| 4 | **무기 수치 테이블** | 데미지·탄속·연사 — Tm.run .data |
| 5 | **이동/선회 키** | GetAsyncKeyState 테이블 |
| 6 | BOB 배치, LFX 의미 확정, (후) 멀티 |
