...
- [x] **attr → 타일 인덱스** 확정: `material×16 + (variant & 0x0F)` (5맵 99.9%+ nonempty)
- [x] **지형 타일 합성 렌더** (`renderMapComposedRgba` + MAP 뷰어「타일 합성」)
- [x] 플레이 **1:1 풀 해상도 지형** (`mapDefFromOriginalFidelity` + 타일 캐시 블릿)
- [x] 기체 **char1–3.spr 120프레임** 인게임 연결 (`src/game/assets.ts`)

### 리버싱으로 확정된 게임 데이터
- [x] 기체 3종: Born Armor / Killers Pot / Sorcerer (`char1–3.spr`, 120프레임=3° yaw)
- [x] 무기 21 이름 + 기체 마스크 (Tm.run `0x39C20`, 23바이트) + body/shot 파일 맵 — wp18–20 없음
- [x] GetAsyncKeyState: SPACE/Alt/ESC/Enter/PgUp·Dn (UI·특수) — 이동은 DirectInput
- [x] 맵 5종 크기·높이 범위 · attr→tile · BOB placed(x,y) 휴리스틱
- [x] 네트워크: UDP sendto/recvfrom, RTT≥1000ms 차단
- [ ] 무기 데미지/연사 바이너리 테이블 (on-disk 미발견)
- [ ] DirectInput 키 바인딩 테이블
...
| 순위 | 작업 | 비고 |
|------|------|------|
| 1 | **무기/기체 수치 런타임 트레이스** | `.data` BSS(~2.7MB) — 정적 덤프 불가, 디버거 필요 (RE_STATUS_2026-08.md) |
| 2 | **DirectInput 키 바인딩** | GAS=UI만 확정 · DI 디바이스 상태 파싱 |
| 3 | **LFX를 SPR/TIL 렌더에 적용** | light×index 리맵 포맷 확정 |
| 4 | **DFX 의미** | 256-entry 타일 리맵 (전 행 동일) |
| 5 | **PPP 디코더** | 헤더 확정 · 페이로드 RLE |
| 6 | BOB 완전 스키마, GM 사운드폰트, 멀티 |
...
**attr:** flags:u8 | 0 | variant:u8 | material:u8 → tile = mat×16+(var&0xF)  
