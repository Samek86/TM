...
### 3.4 미확보 (동일 구현에 필수, 추가 분석 대상)
- 데미지, 연사 간격, 탄속, 사거리, 탄 개수(Vulcan=3 확정), 유도 여부
- 히트박스 / 폭발 반경
- 레벨업 곡선 (Vulcan “레벨업 시 공격력↑”)
- 무기 슬롯 10개와 21종 무기 해금/픽업 규칙
...
`attr` 샘플 패턴: 상위 바이트에 타일 계열, 하위 `0x20`/`0x60` 등 플래그 — **비트필드 의미는 추가 역분석 필요**.
