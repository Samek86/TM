...
import { playWorldWidth, VIEW_WORLD_WIDTH } from "@/game/viewScale";
...
describe("cameraRig", () => {
  ...
  it("uses a tighter frustum only for phone-like play", () => {
    ...
    expect(playWorldWidth(390, false)).toBeCloseTo(VIEW_WORLD_WIDTH * 0.52);
    expect(playWorldWidth(1280, true)).toBeCloseTo(VIEW_WORLD_WIDTH * 0.52);
  });
